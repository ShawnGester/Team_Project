...
public class MealData {
 	 ...
 	 */
  	 private void setUpBPTrees() {
		 trees = new HashMap<String, BPTree<Double, Meal>>();
      		 BPTree<Double, Meal> calories = new BPTree<Double, Meal>(3);
   		 BPTree<Double, Meal> carbs = new BPTree<Double, Meal>(3);
    		 BPTree<Double, Meal> fat = new BPTree<Double, Meal>(3);
     		 BPTree<Double, Meal> fiber = new BPTree<Double, Meal>(3);
      		 BPTree<Double, Meal> protein = new BPTree<Double, Meal>(3);
	...
	/**
    	 ...
    	 * order.
    	 * @param a string to search for within the meal item names
	...
	public List<Meal> filterByNutrient(List<String> rules){
    		...
    		List<List<Meal>> mergeList = new ArrayList<List<Meal>>(); //storing the meals lists from each filter
		try {
    		...
    		List<Meal> retList = new ArrayList<Meal>(mealList);
    		for(List<Meal> filterList : mergeList) { //takes the intersection of the lists of meals from each filter
    			retList.retainAll(filterList);
    		}
    		retList.sort((Meal f1, Meal f2)-> f1.getName().toUpperCase().compareTo(f2.getName().toUpperCase()));
    		return retList;
		}
	}